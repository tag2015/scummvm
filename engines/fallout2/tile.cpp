...
int tileFromScreenXY(int screenX, int screenY, int elevation, bool ignoreBounds) {
	...
	int xTileOff = screenX - _tile_offx - (y * 16);
	...
	if (ignoreBounds || (xPos >= 0 && xPos < gHexGridWidth && yTile >= 0 && yTile < gHexGridHeight)) {
...
void tileRenderFloorsInRect(Rect *rect, int elevation) {
	...
	lightGetAmbientIntensity();  // FIXME this does nothing
