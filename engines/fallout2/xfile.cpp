...
XFile *xfileOpen(const char *filePath, const char *mode) {
	...
	if (drive[0] != '\0' || dir[0] == '\\' || dir[0] == '/' || dir[0] == '.' || strstr(filePath, ".SAV")) {
		...
		debug(5, "XFILE: Absolute path: %s",path);
	} else {
	...
	if (stream->type == XFILE_TYPE_FILE) {
		...
		if (ch1 == 0x1F && ch2 == 0x8B) {
			debug(5, "XFILE: found gzipped stream");
			...
			// File is not gzipped.
			debug(5, "XFILE: found normal (uncompressed) stream");
			stream->file->seek(0, SEEK_SET);
		}
	}
...
size_t xfileRead(void *ptr, size_t size, size_t count, XFile *stream) {
	...
	debug(5, "XFILE: xfileRead: file type: %d", stream->type);
	switch (stream->type) {
	case XFILE_TYPE_DFILE:
	...
	debug(5, "XFILE: elements read: %lld", elementsRead);
