	...
	while (1) {
		...
		if (keyCode == -4) {
			...
			if (itemsLength > listViewCapacity) {
				...
				scrollbarY = (scrollOffset * (listViewMaxY - listViewY - 2 * fontGetLineHeight() - 16 - scrollbarKnobSize - 1)) / (itemsLength - listViewCapacity) + listViewY + fontGetLineHeight() + 7;
...
int win_get_num_i(int *value, int min, int max, bool clear, const char *title, int x, int y) {
	...
	snprintf(hint, 80, "Please enter a number between %d and %d.", min, max);
	windowRefresh(win);
...
int process_pull_down(int win, Rect *rect, char **items, int itemsLength, int foregroundColor, int backgroundColor, MenuBar *menuBar, int pulldownIndex) {
	...
	while (1) {
		...
		if (mx2 < mx1 - 4 || mx2 > mx1 + 4 || my2 < my1 - 4 || my2 > my1 + 4) {
			break;
		}
	...
	while (1) {
		...
		if ((mouseGetEvent() & MOUSE_EVENT_LEFT_BUTTON_UP) != 0 || ((mouseGetEvent() & MOUSE_EVENT_LEFT_BUTTON_DOWN) != 0 && (mouseGetEvent() & MOUSE_EVENT_LEFT_BUTTON_REPEAT) == 0)) {
		...
		if (input == KEY_RETURN) {
			...
			*value = original;
			...
			if (cursorPos != max_chars_wcursor - 1) {
				if ((input == '-' && allow_negative) || (input >= '0' && input <= '9')) {
