...
static int _square_load_scumm(Common::InSaveFile *stream, int a2);
...
// only `self_obj`).
static Common::Array<void *> gMapGlobalPointers; // originally std::vector
...
static Common::Array<void *> gMapLocalPointers; // originally std::vector
...
void mapSetStart(int tile, int elevation, int rotation) {
	debug(5, "mapSetStart - enteringTile %d - new %d", gMapHeader.enteringTile, tile);
...
static char *mapBuildPath(char *name) {
	...
	if (*name != '\\') {
		...
		debug(5, "Built map path: %s", map_path);
		return map_path;
	}
	return name;
}
...
int mapSetEnteringLocation(int elevation, int tile_num, int orientation) {
	debug(5, "mapSetenteringlocation - gEnteringTile %d, new value %d",gEnteringTile, tile_num);
...
void mapNewMap() {
	...
	debug(5, "mapNewMap - enteringTile %d", gMapHeader.enteringTile);
	_obj_remove_all();
	animationStop();
...
int mapLoadByName(char *fileName) {
	...
	if (rc == -1) {
		...
		if (ext != nullptr) {
			...
			debug("MapLoadByName (.map) map path: %s", newPath.c_str());
			...
			if (rc == 0) {
				strncpy(gMapHeader.name, fileName, 15);
...
static int mapLoadScumm(Common::InSaveFile *stream) {
	...
	if (mapHeaderReadScumm(&gMapHeader, stream) != 0) {
		goto err;
	}
	...
	// NOTE: Uninline.
	if (mapGlobalVariablesLoadScumm(stream) != 0) {
		goto err;
	}
	...
	// NOTE: Uninline.
	if (mapLocalVariablesLoadScumm(stream) != 0) {
		goto err;
	}

	if (_square_load_scumm(stream, gMapHeader.flags) != 0) {
		goto err;
	}

	error = "Error reading scripts";
	if (scriptLoadAllScumm(stream) != 0) {
		goto err;
	}

	error = "Error reading objects";
	if (objectLoadAllScumm(stream) != 0) {
		goto err;
	}
...
static int mapLoad(File *stream) {
	...
	debug(5, "MapLoad - gEnteringElevation %d", gEnteringElevation);
	if (gEnteringElevation == -1) {
		// NOTE: Uninline.
		mapSetEnteringLocation(gMapHeader.enteringElevation, gMapHeader.enteringTile, gMapHeader.enteringRotation);
	}
...
static int _map_age_dead_critters() {
	...
	while (obj != nullptr) {
		if (PID_TYPE(obj->pid) == OBJ_TYPE_CRITTER && obj != gDude && !objectIsPartyMember(obj) && !critterIsDead(obj)) {
	...
	for (int index = 0; index < count; index++) {
		...
		if (PID_TYPE(obj->pid) == OBJ_TYPE_CRITTER) {
			...
			if ((proto->critter.flags & CRITTER_FLAT)) {
				...
				if (critterGetKillType(obj) != KILL_TYPE_RAT && critterGetKillType(obj) != KILL_TYPE_MANTIS) {
					frame += 3;
				}
			}
...
static int _map_save_file(Common::OutSaveFile *stream) {
	...
	debug(5, "Saved map header and data");
...
int _map_save_in_game(bool a1) {
	...
	debug(5, "Attempting map_save_in_game! a1 = %d", a1);
...
static int _square_load_scumm(Common::InSaveFile *stream, int flags) {
	int v6;
	int v7;
	int v8;
	int v9;
