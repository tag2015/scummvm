...
int combatSave(Common::OutSaveFile *stream) {
	...
	for (int index = 0; index < _list_total; index++) {
		...
		stream->writeSint32BE(aiInfo->friendlyDead != NULL ? aiInfo->friendlyDead->id : -1);
		stream->writeSint32BE(aiInfo->lastTarget != NULL ? aiInfo->lastTarget->id : -1);
		stream->writeSint32BE(aiInfo->lastItem != NULL ? aiInfo->lastItem->id : -1);
...
void _combat(CombatStartData *csd) {
	...
	if (csd == NULL || (csd->attacker == NULL || csd->attacker->elevation == gElevation) || (csd->defender == NULL || csd->defender->elevation == gElevation)) {
...
void _compute_explosion_on_extras(Attack *attack, bool isFromAttacker, bool isGrenade, bool noDamage) {
	...
	while (attack->extrasLength < maxTargets) {
		...
		if (obstacle != NULL && FID_TYPE(obstacle->fid) == OBJ_TYPE_CRITTER &&
