...
//		return -1;
	...
	_combat_turn_running = stream->readSint32BE();
	_combat_free_move = stream->readSint32BE();
	_combat_exps = stream->readSint32BE();
...
int combatSave(Common::OutSaveFile *stream) {
	...
	stream->writeSint32BE(_combat_turn_running);
	stream->writeSint32BE(_combat_free_move);
	stream->writeSint32BE(_combat_exps);
	stream->writeSint32BE(_list_com);
	stream->writeSint32BE(_list_noncom);
	...
	for (int index = 0; index < _list_total; index++) {
		...
		stream->writeSint32BE(aiInfo->friendlyDead != nullptr ? aiInfo->friendlyDead->id : -1);
		stream->writeSint32BE(aiInfo->lastTarget != nullptr ? aiInfo->lastTarget->id : -1);
		stream->writeSint32BE(aiInfo->lastItem != nullptr ? aiInfo->lastItem->id : -1);
		stream->writeSint32BE(aiInfo->lastMove);
...
void _combat(CombatStartData *csd) {
	...
	if (csd == nullptr || (csd->attacker == nullptr || csd->attacker->elevation == gElevation) || (csd->defender == nullptr || csd->defender->elevation == gElevation)) {
...
void _compute_explosion_on_extras(Attack *attack, bool isFromAttacker, bool isGrenade, bool noDamage) {
	...
	while (attack->extrasLength < maxTargets) {
		...
		if (obstacle != nullptr && FID_TYPE(obstacle->fid) == OBJ_TYPE_CRITTER &&
			(obstacle->data.critter.combat.results & DAM_DEAD) == 0 && (obstacle->flags & OBJECT_SHOOT_THRU) == 0 &&
...
static void attackComputeDamage(Attack *attack, int ammoQuantity, int bonusDamageMultiplier) {
	...
	*damagePtr = 0;
	...
	if (knockbackDistancePtr != nullptr && (critter->flags & OBJECT_MULTIHEX) == 0 && (damageType == DAMAGE_TYPE_EXPLOSION || attack->weapon == nullptr || weaponGetAttackTypeForHitMode(attack->weapon, attack->hitMode) == ATTACK_TYPE_MELEE) && PID_TYPE(critter->pid) == OBJ_TYPE_CRITTER && !_critter_flag_check(critter->pid, CRITTER_NO_KNOCKBACK)) {
...
void _combat_display(Attack *attack) {
	...
	char text[280];
	if (attack->defender != nullptr && attack->oops != nullptr && attack->defender != attack->oops && (attack->attackerFlags & DAM_HIT) != 0) {
