...
void _gdialog_scroll_subwin(int windowIdx, bool scrollUp, unsigned char *windowFrmData, unsigned char *windowBuf, unsigned char *bgWindowFrmData, int windowHeight, bool instantScrollUp) {
	...
	if (scrollUp) {
		...
		for (int strips = windowHeight / 10; strips > 0; strips--) {
			sharedFpsLimiter.mark();
...
int partyMemberControlWindowInit() {
	...
	_gdialog_scroll_subwin(gGameDialogWindow, true, backgroundData, windowBuffer, 0, _dialogue_subwin_len);
	backgroundFrmImage.unlock();
...
int _gdCustomSelect(int a1) {
	...
	int btn1 = buttonCreate(win, 70, 164, _littleRedButtonNormalFrmImage.getWidth(), _littleRedButtonNormalFrmImage.getHeight(),
							-1, -1, -1, 500, _littleRedButtonNormalFrmImage.getData(), _littleRedButtonPressedFrmImage.getData(), nullptr, BUTTON_FLAG_TRANSPARENT);
	if (btn1 == -1) {
		windowDestroy(win);
		return -1;
	} else {
