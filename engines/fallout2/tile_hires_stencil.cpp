...
static Point get_screen_diff() {
...
#ifdef DO_DEBUG_CHECKS
	{
		...
		if (minTileX != hex_tile_with_lowest_x || minTileY != hex_tile_with_lowest_y) {
			warning("min tileX=%i x=%i tileY=%i y=%i maxX=%i maxY=%i dx=%i dy=%i\n",
					minTileX, minX,
					minTileY, minY,
					maxX, maxY,
					maxX - minX, maxY - minY);
			error("get_screen_diff - errorcode 108");
		}
	};
#endif
...
static void mark_screen_tiles_around_as_visible(int center_tile, const Point &screen_diff, MarkOnlyPart part) {
...
#ifdef DO_DEBUG_CHECKS
	{
		if (centerTileGlobalX % square_width != 0 || centerTileGlobalY % square_height != 0) {
			warning("centerTileGlobalX=%i, centerTileGlobalY=%i\n", centerTileGlobalX, centerTileGlobalY);
			error("mark_screen_tiles_around_as_visible - errorcode 101");
		}

		if (centerTileGlobalX - squares_screen_width_half * square_width < 0 || centerTileGlobalY - squares_screen_height_half * square_height < 0) {
			warning("centerTileGlobalX=%i, centerTileGlobalY=%i\n", centerTileGlobalX, centerTileGlobalY);
			warning("kekekek border=%i\n", gTileBorderInitialized);
			error("mark_screen_tiles_around_as_visible - errorcode 102");
		}
		if (centerTileGlobalX + squares_screen_width_half * square_width >= square_width * square_grid_width ||
			centerTileGlobalY + squares_screen_height_half * square_height >= square_height * square_grid_height) {
			warning("centerTileGlobalX=%i, centerTileGlobalY=%i\n", centerTileGlobalX, centerTileGlobalY);
			error("mark_screen_tiles_around_as_visible - errorcode 103");
		}
	}
#endif
	...
	int horizontal_start = part != MarkOnlyPart::RIGHT ? horizonal_start_full : horizonal_end_full - squares_per_horizontal_move;
	int horizontal_end = part != MarkOnlyPart::LEFT ? horizonal_end_full : horizonal_start_full + squares_per_horizontal_move;
	int vertical_start_full = squareY - squares_screen_height_half;
	int vertical_end_full = squareY + squares_screen_height_half;

	int vertical_start = part != MarkOnlyPart::DOWN ? vertical_start_full : vertical_end_full - squares_per_vertical_move;
	int vertical_end = part != MarkOnlyPart::UP ? vertical_end_full : vertical_start_full + squares_per_vertical_move;
...
void tile_hires_stencil_on_center_tile_or_elevation_change() {
	...
	Common::Array<struct TileToVisit> tiles_to_visit; // originally std::vector
