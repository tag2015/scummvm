...
void handlePipboyPageNavigation(int mouseX, int threshold, int *viewPage, int totalPages, void (*handleAutomaps)(int), void (*updatePage)(void));
...
void handlePipboyPageNavigation(int mouseX, int threshold, int *viewPage, int totalPages, void (*handle_outofRange)(int), void (*updatePage)(void)) {
...
static bool pipboyRest(int hours, int minutes, int duration) {
	...
	if (duration == 0) {
		...
		if (currentHp != maxHp || (duration == PIPBOY_REST_DURATION_UNTIL_PARTY_HEALED && partyIsAnyoneCanBeHealedByRest())) {
